...
// Variables
    ...
    int nodeListLen
        ...
        // this.schedule = new ArrayList<ArrayList<MyMac.ScheduleSlot>>()
        // this.transmissionSlotsList = new ArrayList<MyMac.TransmissionSlot>()
        // this.nodeList = new ArrayList<Integer>()

    List<Integer> nodeList = new ArrayList<Integer>()
    List<Integer> maxDelay = new ArrayList<Integer>();
    List<ArrayList<MyMac.ScheduleSlot>> schedule = new ArrayList<ArrayList<MyMac.ScheduleSlot>>();
    List<MyMac.TransmissionSlot> transmissionSlotsList = new ArrayList<MyMac.TransmissionSlot>()
    ...
    public void startup() {
        ...
        myAddr = node.Address
        this.nodeList.add(myAddr)
        this.nodeListLen = this.nodeList.size()
        nodePosition = this.nodeList.indexOf(myAddr)
    ...
    private void setNodeList(List nodeList){
        println "HELLO"
        assert this != null
        assert this.nodeList != null
        this.nodeList = nodeList
        println nodeList.size().toString()
        println "EVERYWHERE"
        nodeCount = nodeList.size()
        this.nodeListLen = nodeList.size()
    ...
    private void initilialisationPhase(){
        ...
        if(scheduleSize > nodeCount){
            ...
            {
                schedule.add(new ArrayList<MyMac.ScheduleSlot>())
            }
        }   
    ...
    private void sendNtf(ReservationReq msg){
        ...
        //check if transmission can result in collisions                
            ...
            //check physically if modem is busy
                receivingNtfCheck()
                //if these conditions are met , it assumes modem is busy receving a ntf packet
    ...
    private void sendData(TransmissionSlot tSlot)   {
        ...
        add new WakerBehavior(lagTime, {           
            if(tSlot != null){
                ...
                if(currentTime < tSlot.startTimeData){ 
                //this implies that the transmission was deferred due to a clash with another node implied by a ntf packet
    ...
    private void backoff(int bo, TransmissionSlot tSlot){
        ...
        add new BackoffBehavior(bo*dataMsgDuration, {
            if(tSlot != null){
                int currentTime = getCurrentTime()  
                if(currentTime < tSlot.startTimeNtf){
                    //this implies that the transmission was deferred due to a clash with another node implied by a ntf packet
                    int start_time = (Integer)(tSlot.startTimeNtf)
                    // long diff = tSlot.startTimeNtf - currentTime;
                    int bo1 = start_time - currentTime
                    backoff(bo1)
                }else{
                    transmissionClashCheckFlag = transmissionClashCheck(NTF_PACKET) 
                    if (transmissionClashCheckFlag != 0){
    ...
    private void rxDisable(){
        ...
        ParameterRsp rsp = (ParameterRsp) rindexOf
    ...
    public void processMessage(Message msg) {
        ...
        if(msg instanceof RxFrameNtf){       
            ...
            if( msg.getTo() == Address.BROADCAST ){
                ...
                int sourcePosition     = this.nodeList.indexOf(msg.getFrom()) 

                def pkt = ntfMsg.decode(msg.data)   
                int destinationPosition = this.nodeList.indexOf(pkt.destinationNodeAddress)
    ...
    List<Parameter> getParameterList() {
        return allOf(MacParam,MyParams)                            // advertise the list of parameters
    }

    boolean channelBusy = false                   // parameters are marked as 'final'
    int reservationPayloadSize = 0                //  to ensure that they are read-only
    int ackPayloadSize = 0
    float maxReservationDuration = Float.POSITIVE_INFINITY
    Float recommendedReservationDuration = null
...
enum MyParams implements Parameter {
