 ...
 * IEEE INFOCOM 2007.
 */
class OldAlohaAN extends UnetAgent {
    ...
    ArrayList<ArrayList<OldAlohaAN.ScheduleSlot>> schedule  = new ArrayList<ArrayList<OldAlohaAN.ScheduleSlot>>() 
    ArrayList<OldAlohaAN.TransmissionSlot> transmissionSlotsList = new ArrayList<OldAlohaAN.TransmissionSlot>()
    ...
    public void initParams(address_list, node_locations, channel, modem){
        this.setNodeList(address_list)
        ...
        def nodes = []
        for(int i = 1; i<=node_locations.size(); i++){
            nodes.add(i)
        }
        // print 'Node Locaitons'
        // print node_locations.toString()
        // print 'Nodes'
        // print nodes.toString()
        // print 'Address List'
        // print address_list.toString()
        ...
        def propDel = [nodes.size()][nodes.size()]
        print propDel
        // print nodes
        // print "NODES"
        // nodes.each{ x ->
        ...
        for(int n1 = 0; n1 < this.nodeCount; n1++){
            ...
            for(int n2 = 0; n2 < this.nodeCount; n2++){
                ...
                print "here" 
                propDel[n1][n2] = (int)( (distance(node_locations[n1],node_locations[n2]) * 1000 / channel.soundSpeed) + 0.5 ) 
                print "here" 
            }
            print propDel
        }
        ...
        for(int i = 0;i<this.nodeCount;i++){
            for(int j = 0;j<this.nodeCount;j++){
                if(propDel[i][j] != propDel[j][i]){
                    log.warning 'ERROR IN PROPAGATION_DELAY_PARAMETER'
                }else if(i == j && propDel[i][i] != 0.0){
                    log.warning 'ERROR IN PROPAGATION_DELAY_PARAMETER'
                }else{
                    print "HIT"
                    print i
                    print j
                    this.propagationDelay.add(propDel[i][j])                                     
                }            
            }
        } 
    ...
    private void initilialisationPhase()
    {
        ...
        else
        {
            ...
            {
                schedule.add(new ArrayList<OldAlohaAN.ScheduleSlot>())
            }
        }   
